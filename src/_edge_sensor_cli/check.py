...
def run(yaml_path):
    ...
    print(60*'=')
    all_fields = _get_default_format_fields(sweep, radio_id=radio_id, yaml_path=yaml_path)
    fields = {k: v for k,v in all_fields.items() if k == 'start_time' or k not in sweep.defaults.__struct_fields__}
