...
def simulated_awgn(duration: float, sample_rate: float, power: float = 1, xp=np, pinned_cuda=False):
    try:
        # e.g., numpy
    ...
    generator.standard_normal(size=2 * size, dtype=xp.float32, out=samples.view(xp.float32))
