...
CellularFrame = Annotated[
    Union[str, None],
    Meta("string composed of ('d', 'u', 's') specifying the sequence of slots in 1 TDD cellular frame, or None to fill with downlink")
]
CellularSpecialSymbols = Annotated[
    Union[str, None],
    Meta("string composed of the characters ('d', 'u', 'f') indicating the sequence of symbol types (when 's' is in frame_slots)")
]
CellularSubcarrierSpacing = Annotated[
    float,
    Meta('Subcarrier spacing (15e3, 30e3, 60e3, etc)', units='Hz')
]
...
CellularCyclicPrefix = Annotated[
    Union[Literal['normal'], Literal['extended']],
    Meta('the 3GPP cyclic prefix type')
]
...
CellularAverageRBs = Annotated[
    Union[bool, Literal['half']], Meta('True (or "half") to coarsen spectrogram bins by integrating 1-subcarrier frequency resolution into 1- (or 1/2)-resource block')
]
...
LOBandstop = Annotated[
    float,
    Meta('mask with float("nan") at this bandwidth centered at baseband DC', units='Hz')
]
...
PowerBinMin = Annotated[
    float,
    Meta('Minimum power bin', units='dB power')
]
PowerBinMax = Annotated[
    float,
    Meta('Maximum power bin', units='dB power')
]
PowerBinStep = Annotated[
    float,
    Meta('Power bin resolution', units='dB')
]
