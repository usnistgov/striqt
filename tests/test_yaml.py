...
def test_read(spec_dir):
    sensor.read_yaml_spec(spec_dir/'cw-cpu.yaml')


def test_yaml_schema_binding(spec_dir):
    spec = sensor.read_yaml_spec(spec_dir/'cw-cpu.yaml')
...
def test_binding_args():
    union = sensor.lib.bindings.get_tagged_sweep_type()
    for struct in union.__args__:  # type: ignore
        msgspec.inspect.type_info(struct)
        assert issubclass(struct, sensor.specs.Sweep)
